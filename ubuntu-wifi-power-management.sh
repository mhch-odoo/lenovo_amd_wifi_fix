...
if [ "$WIFI_DRIVER" != "ath11k_pci" ]; then
    log 3 "This script is intended for systems using the ath11k_pci Wi-Fi driver."
    log 3 "Detected Wi-Fi driver: $WIFI_DRIVER. Please contact mhch@odoo.com for assistance."
    exit 1
fi
...
TEMP_DIR=$(mktemp -d)
curl -s -o $TEMP_DIR/reset-wifi-module.sh https://raw.githubusercontent.com/s-damian/thinkpad-t14-gen-5-amd-linux/refs/heads/main/sh/reset-wifi-module.sh
if [ $? -ne 0 ]; then
